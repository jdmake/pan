...
function open_tab(title, url) {
	...
	if($('.nav-tabs a[data-url="'+ url +'"]').length) {
		$('.nav-tabs a[data-url="'+ url +'"]').click();
	}
	else {
		$('#nav-tabs li').removeClass('active');
		var id = $('#nav-tabs li').length + 1;
		var tab = $('<li class="active" >\n' +
			'<a data-url="'+ url +'" href="#tab-'+ id +'" data-toggle="tab" style="display: flex; align-items: center; padding-right: 0px;">\n' +
			'<span class="hidden-xs">'+ title +'</span>\n' +
			'<div style="padding: 0 10px; margin-left: 5px; cursor: pointer" href=""><i class="fa-close"></i></div>\n' +
			'</a>\n' +
			'</li>');
		tab.find('.fa-close').on('click', function () {
			tab.remove();
			$('#tab-content #tab-'+ id).remove();
			$('.nav-tabs li').removeClass('active');
			$('.nav-tabs li').eq($('.nav-tabs li').length-1).find('a').click();
		});
		$('#nav-tabs').append(tab);
		$('#tab-content .tab-pane').removeClass('active');
		var content = $('<div data-url="'+ url +'"  class="tab-pane active" id="tab-'+ id +'"></div>');
...
function open_modal(title, url, trigger) {
	$('#modal-6').modal('show', {backdrop: 'static'});
	$('#modal-6').find('.modal-title').html(title);
	$('#modal-6').find('.modal-body').html('').load(url, function () {
		window.__reload_ele = trigger;
	});
}
...
function reloadCurrentTabContent() {
	window.__reload_ele.trigger('click');
}
